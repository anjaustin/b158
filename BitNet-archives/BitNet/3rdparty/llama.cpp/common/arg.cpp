...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--dither", "--dither-intensity"}, "N",
        string_format("ordered dithering intensity (default: %.4f = 1/137 fine structure constant, 0.0 = disabled, range: [α, φ/10])", (double)params.sparams.dither_intensity),
        [](common_params & params, const std::string & value) {
            params.sparams.dither_intensity = std::stof(value);
        }
    ).set_sparam());
    add_opt(common_arg(
        {"--halo"},
        "enable Halo multi-pass synthesis mode (3+3+1+1 = 8 generations with divergent/convergent phases)",
        [](common_params & params) {
            params.halo_mode = true;
        }
    ));
    add_opt(common_arg(
        {"--floyd-steinberg", "--fs"}, "N",
        string_format("Floyd-Steinberg error diffusion intensity (default: %.2f, 0.0 = disabled)\n"
        "Single-pass algorithm: error from each token propagates forward to sharpen subsequent decisions", 
        (double)params.sparams.fs_intensity),
        [](common_params & params, const std::string & value) {
            params.sparams.fs_intensity = std::stof(value);
        }
    ).set_sparam());
    add_opt(common_arg(
        {"--fs-pass2", "--fs2"},
        "Enable Floyd-Steinberg two-pass mode:\n"
        "Pass 1: Generate draft with F-S, collect error map\n"
        "Pass 2: Regenerate with draft as context, apply position-matched sharpening",
        [](common_params & params) {
            params.fs_two_pass = true;
        }
    ));
    add_opt(common_arg(
        {"--mirostat"}, "N",
