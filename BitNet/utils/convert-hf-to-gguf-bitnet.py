    ...
    def get_vocab_base(self) -> tuple[list[str], list[int], str]:
        ...
        for i in range(vocab_size):
            if i not in reverse_vocab:
                ...
                encoded_decoded_token = tokenizer.decode(
                    tokenizer.encode(reverse_vocab[i])
                )
...
def process_tl1(weight, BM, BY, bm, by, M, K):
    ...
    weight = weight.reshape((M // BM, K // BY, BM // bm, bm, BY // 2)).transpose(
        0, 1, 2, 4, 3
    )
    weight = weight.reshape(
        (M // BM, K // BY, BM // bm, BY // by, by // 2, bm)
    ).transpose(0, 1, 2, 3, 5, 4)
    weight = weight.reshape(
        (M // BM, K // BY, BM // bm, BY // by, bm // 16, 16, by // 2)
    ).transpose(0, 1, 2, 3, 4, 6, 5)
    weight = weight.reshape(
        (M // BM, K // BY, BM // bm, BY // by, bm // 16, by // 4, 4 // 2, 16)
...
def preprocess_three_weights_tl2(
    ...
    weight = weight.reshape((M // BM, K // BY, BM // bm, bm, BY // 3)).transpose(
        0, 1, 2, 4, 3
    )
    weight = weight.reshape(
        (M // BM, K // BY, BM // bm, BY // by, by // 3, bm)
    ).transpose(0, 1, 2, 3, 5, 4)
    ...
    sign_weight = sign_weight.reshape((M // BM, K // BY, BY // 3, BM)).transpose(
        0, 1, 3, 2
    )
    sign_weight = sign_weight.reshape(
        (M // BM, K // BY, BM // bm, bm, BY // 3)
    ).transpose(0, 1, 2, 4, 3)
    sign_weight = sign_weight.reshape(
        (M // BM, K // BY, BM // bm, BY // (by * 4), by // 3 * 4, bm)
    ).transpose(0, 1, 2, 3, 5, 4)
    sign_weight = sign_weight.reshape(
        (M // BM, K // BY, BM // bm, BY // (by * 4), bm, by // 3 * 4)
    ).transpose(0, 1, 2, 3, 5, 4)
    sign_weight = sign_weight.reshape(
        (M // BM, K // BY, BM // bm, BY // (by * 4), by // 3 * 8, bm // 2)
    ).astype(np.uint16)
    combine_weight = np.zeros(
        (M // BM, K // BY, BM // bm, BY // (by * 4), bm // 2), dtype=np.uint16
    )
    for i in range(16):
...
def preprocess_weights_tl2(
    ...
    weight = np.pad(
        weight,
        (0, (K - 256) * M // 3 * 5 // 8 + 256 * M // 2 * 4 // 8 - weight.shape[0]),
        mode="constant",
        constant_values=0,
    )
    return weight
    ...
    def write_tensors(self):
        ...
        for name, data_torch in self.get_tensors():
            ...
            if name.replace(".weight", "") in scale_map:
                ...
                data_torch = (data_torch.float() - 1).reshape(
                    (origin_shape[0] * 4, *origin_shape[1:])
                )
            ...
            for new_name, data in (
                ...
                self.gguf_writer.add_tensor(
                    new_name, data, raw_shape=shape, raw_dtype=data_qtype
                )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--quant-embd", action="store_true", help="quantize the embedding layer"
    )
