...
// dither (ordered dithering with adaptive intensity based on logit variance)

struct llama_sampler_dither {
    const float intensity;       // base intensity when variance is zero
    const float variance_scale; // how much to scale by variance
    const bool adaptive;        // use adaptive intensity based on variance
    int32_t position;
};

static const char * llama_sampler_dither_name(const struct llama_sampler * /*smpl*/) {
    return "dither";
}

static float compute_logit_variance(llama_token_data_array * cur_p) {
    // Compute mean of logits
    float sum = 0.0f;
    for (size_t i = 0; i < cur_p->size; ++i) {
        sum += cur_p->data[i].logit;
    }
    float mean = sum / cur_p->size;

    // Compute variance
    float sum_sq = 0.0f;
    for (size_t i = 0; i < cur_p->size; ++i) {
        float diff = cur_p->data[i].logit - mean;
        sum_sq += diff * diff;
    }
    return sum_sq / cur_p->size;
}

static void llama_sampler_dither_apply(struct llama_sampler * smpl, llama_token_data_array * cur_p) {
    auto * ctx = (llama_sampler_dither *) smpl->ctx;

    if (ctx->intensity <= 0.0f || cur_p->size == 0) {
        return;
    }

    int32_t pos = ctx->position++;

    // Compute adaptive intensity based on logit spread
    float scale = ctx->intensity;
    if (ctx->adaptive) {
        // Compute spread as (max - min) instead of variance for stability
        ...
        for (size_t i = 1; i < cur_p->size; ++i) {
            ...
            if (cur_p->data[i].logit < min_logit) min_logit = cur_p->data[i].logit;
        }
        float spread = max_logit - min_logit;
        
        // Clamp spread
        if (spread < 1.0f) spread = 1.0f;
        if (spread > 50.0f) spread = 50.0f;
        
        // Normalize: low spread = low confidence = high dither
        // High spread = high confidence = low dither
    ...
    for (size_t i = 0; i < cur_p->size; ++i) {
        ...
        dither = bayer[x & 3][y & 3] - 0.5f;

        cur_p->data[i].logit += dither * scale;
    }

    cur_p->sorted = false;
}

static void llama_sampler_dither_reset(struct llama_sampler * smpl) {
    auto * ctx = (llama_sampler_dither *) smpl->ctx;
    ctx->position = 0;
}

static struct llama_sampler * llama_sampler_dither_clone(const struct llama_sampler * smpl) {
    const auto * ctx = (const llama_sampler_dither *) smpl->ctx;
    if (ctx->adaptive) {
        return llama_sampler_init_dither_adaptive(ctx->intensity, ctx->variance_scale);
    }
    return llama_sampler_init_dither(ctx->intensity);
}

static void llama_sampler_dither_free(struct llama_sampler * smpl) {
    delete (llama_sampler_dither *) smpl->ctx;
}

static struct llama_sampler_i llama_sampler_dither_i = {
    /* .name   = */ llama_sampler_dither_name,
    /* .accept = */ nullptr,
    /* .apply  = */ llama_sampler_dither_apply,
    /* .reset  = */ llama_sampler_dither_reset,
    /* .clone  = */ llama_sampler_dither_clone,
    /* .free   = */ llama_sampler_dither_free,
};

// Basic dither (fixed intensity)
struct llama_sampler * llama_sampler_init_dither(float intensity) {
    return new llama_sampler {
        /* .iface = */ &llama_sampler_dither_i,
        /* .ctx   = */ new llama_sampler_dither {
            /* .intensity      = */ intensity,
            /* .variance_scale = */ 0.0f,
            /* .adaptive       = */ false,
            /* .position       = */ 0,
...
// Adaptive dither (intensity scales inversely with logit variance)
struct llama_sampler * llama_sampler_init_dither_adaptive(float intensity, float variance_scale) {
    return new llama_sampler {
        /* .iface = */ &llama_sampler_dither_i,
        /* .ctx   = */ new llama_sampler_dither {
            /* .intensity      = */ intensity,
