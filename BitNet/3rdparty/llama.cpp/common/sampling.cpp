...
struct common_sampler * common_sampler_init(const struct llama_model * model, const struct common_sampler_params & params) {
    ...
    if (params.temp > 0.0f) {
        if (params.mirostat == 0) {
            ...
            if (params.dither > 0.0f) {
                llama_sampler_chain_add(result->chain, llama_sampler_init_dither(params.dither));
            }
            if (params.dither_adaptive > 0.0f) {
                llama_sampler_chain_add(result->chain, llama_sampler_init_dither_adaptive(params.dither_adaptive, 0.5f));
            }
