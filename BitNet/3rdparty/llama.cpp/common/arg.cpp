...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--dither"}, "N",
        string_format("dithering intensity (default: %.1f, 0.0 = disabled)", (double)params.sparams.dither),
        [](common_params & params, const std::string & value) {
            params.sparams.dither = std::stof(value);
            params.sparams.dither = std::max(params.sparams.dither, 0.0f);
        }
    ).set_sparam());
    add_opt(common_arg(
        {"--dither-adaptive"}, "N",
        string_format("adaptive dithering intensity (default: %.1f, scales with variance)", (double)params.sparams.dither_adaptive),
        [](common_params & params, const std::string & value) {
            params.sparams.dither_adaptive = std::stof(value);
            params.sparams.dither_adaptive = std::max(params.sparams.dither_adaptive, 0.0f);
        }
    ).set_sparam());
    add_opt(common_arg(
        {"--top-k"}, "N",
